...
class TransportLink:
    def __init__(self, env, source, dest, mode):
        self.env = env
        self.source = source
        self.dest = dest
        self.mode = mode
...
class RoadLink(TransportLink):
    def __init__(self, env, src, dst, google_client=None,
                 noise_mean_h=0.5, noise_std_h=0.2,
                 fallback_mean_h=15.0, fallback_std_h=3.0,
                 base_cost=250.0, cost_per_hour=12.0):
    ...
    def traverse(self, shipment):
        if hasattr(self, '_pre_delay'):
            ...
            api_hours = self.google_client.get_travel_time_hours(
                self.source, self.dest, self.env.now)
            noise = 0.0
...
class RailLink(TransportLink):
    def __init__(self, env, src, dst, rail_client=None,
                 min_hours=20.0, max_hours=30.0,
                 base_cost=500.0, cost_per_hour=5.0):
...
class AirLink(TransportLink):
    def __init__(self, env, src, dst, air_client=None,
                 mean_hours=5.0, std_hours=0.5,
                 base_cost=2000.0, cost_per_hour=50.0):
    ...
    def traverse(self, shipment):
        if hasattr(self, '_pre_delay'):
            ...
            base_h = self.air_client.get_transit_time_hours(
                self.source, self.dest) if self.air_client else self.mean_hours
...
class ShipLink(TransportLink):
    def __init__(self, env, src, dst, sea_client=None,
                 min_hours=100.0, mode_hours=120.0, max_hours=180.0,
                 fuel_cost=1500.0, cost_per_hour=2.5):
